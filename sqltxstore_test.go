...
type SQLTxStoreTestSuite struct {
	MessageStoreTestSuite
}

func (suite *SQLTxStoreTestSuite) SetupTest() {
	testName := suite.getTestName(suite.T())
	sqlStoreRootPath := path.Join(suite.T().TempDir(), fmt.Sprintf("SqlTxStoreTestSuite-%s-%d", testName, os.Getpid()))
...
TargetCompID=%s`, sqlDriver, sqlDsn, sessionID.BeginString, sessionID.SenderCompID, sessionID.TargetCompID)))
	...
	// create store
	suite.msgStore, err = NewSQLTxStoreFactory(settings).Create(sessionID)
	require.Nil(suite.T(), err)
}

func (suite *SQLTxStoreTestSuite) TearDownTest() {
	suite.msgStore.Close()
	// os.RemoveAll(suite.getDirPath(suite.T()))
}

func TestSqlTxStoreTestSuite(t *testing.T) {
	suite.Run(t, new(SQLTxStoreTestSuite))
}
